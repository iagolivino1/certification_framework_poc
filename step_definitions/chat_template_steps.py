...
def set_pages(driver):
    ...
    CHAT_TEMPLATE = ChatTemplatePage(driver)


@when("I go to chat template page")
def open_chat_page(driver2):
    set_pages(driver2)
    CHAT_TEMPLATE.open_page()
    common.wait_element_to_be_clickable(driver2, CHAT_TEMPLATE.open_chat_button)


@when("I start chat interaction")
def start_chat_interaction(driver2):
    CHAT_TEMPLATE.get_title_input().send_keys('Automatic chat')
    common.element_recursive_click(driver2, CHAT_TEMPLATE.open_chat_button, 2)
    common.switch_to_frame(driver2, CHAT_TEMPLATE.get_chat_frame())
    common.wait_page_element_load(driver2, CHAT_TEMPLATE.name_input)
...
def send_message_to_agent(driver2, start_message):
    common.wait_page_element_load(driver2, CHAT_TEMPLATE.start_chat_button)
    common.system_wait(2)
    CHAT_TEMPLATE.get_question_textarea().send_keys(start_message)
    common.system_wait(2)
    CHAT_TEMPLATE.get_start_chat_button().click()
    common.wait_page_element_load(driver2, CHAT_TEMPLATE.loading_message)


@when("I check if agent message is displayed in customer chat")
def check_agent_message(driver2, reply_message):
...
def check_closed_chat(driver2):
    driver2.switch_to.window(driver2.current_window_handle)
    common.switch_to_frame(driver2, CHAT_TEMPLATE.get_chat_frame())
    common.wait_page_element_load(driver2, CHAT_TEMPLATE.send_survey_button)
...
def close_chat_and_page(driver2):
    ...
    common.wait_page_element_load(driver2, CHAT_TEMPLATE.end_chat_popup)
